...
async function getStatic(pathname, publicFolder) {
  const alternatives = [pathname, ...alternativePathsFor(pathname)].map((pathnames) =>
    path.resolve(publicFolder, pathnames.slice(1))
  )
...
function initializeProxy(port, distDir, projectDir) {
  ...
  const handlers = {
    web: (req, res, options) => {
      ...
      req.alternativePaths = alternativePathsFor(requestURL.pathname).map((path) => path + requestURL.search)
